...
\begin{table}[t]
	...
	\begin{tabular}{l c c c >{\raggedright\arraybackslash}p{4cm}}
		...
		\textbf{DEV-NG-02} & Yes & Yes & Attempted & Low emission volumes, high costs, insufficient revenue \\
		\textbf{DEV-KE-01} & Yes & No & Pursuing & Methodology challenges, high consultant costs, small portfolio \\
...
To assess the disproportionate burden of transaction costs on different project sizes, Table~\ref{tab:pct-costs} breaks down certification costs against typical mini-grid capital expenditure. Capital expenditure (CAPEX) values assume the average mini-grid cost in SSA is approximately 6,824~USD/kWp.~\cite{amda-bam}
...
This analysis demonstrates that smaller projects face a higher relative burden of certification costs, underscoring the disproportionate impact of transaction costs on small-scale mini-grid developers. For mini-grids under 200~kWp, certification costs represent approximately 10\% of total capital expenditure, which developers consistently identified as a significant constraint on project feasibility (DEV-NG-02).
...
Multiple respondents identified Renewable Energy Certificates as potentially more viable. REG-01 advised ``RECs are a safer choice, better regulated, less change and rules around them.'' DEV-KE-02 cited the R-REC Standard~\cite{rrecs} with ``methodology in line with UNFCCC guidelines that allows you to convert RECs to carbon credits,'' representing hybrid flexibility. Platform developments integrating ``[I-TRACK's International Renewable Energy Certificates] (I-RECs), [Tradeable Instruments for Global Renewables] (TIGRs), and [Global Energy Ownership certificates] (GEOs) onto the same platform as carbon credits'' could increase liquidity (EXP-02). REG-01 noted branded Distributed Renewable Energy Certificates (D-RECs) command premium prices ``because of their use in specialized areas'' like schools and clinics, suggesting co-benefit documentation enhances value.
...
\begin{enumerate}
	\item Market forces do not produce necessary aggregation infrastructure. MiniGridCo succeeded because atmosfair possesses institutional endowments, such as Development Finance Institution (DFI) backing, technical expertise, patient capital, unavailable to commercial developers, not because PoA mechanisms are accessible.
...
\begin{table}[t]
	\centering
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{l >{\raggedright\arraybackslash}p{4cm} c >{\raggedright\arraybackslash}p{4cm} >{\raggedright\arraybackslash}p{4cm}}
		\toprule
